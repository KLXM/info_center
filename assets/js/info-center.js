...
(function() {
    ...
    function getCurrentArticleInfo() {
        // Versuche aktuelle Artikel-Information aus der URL zu extrahieren
        const urlParams = new URLSearchParams(window.location.search);
        const articleId = urlParams.get('article_id') || urlParams.get('category_id');
        const clangId = urlParams.get('clang');
        
        // Alternative: aus dem DOM extrahieren
        if (!articleId) {
            // Suche nach versteckten Inputs mit Artikel-Info
            const articleInput = document.querySelector('input[name="article_id"]');
            const categoryInput = document.querySelector('input[name="category_id"]');
            const clangInput = document.querySelector('input[name="clang"]');
            
            return {
                articleId: articleInput ? articleInput.value : (categoryInput ? categoryInput.value : null),
                clangId: clangInput ? clangInput.value : null
            };
        }
    ...
    function refreshArticleWidgetContent(widget) {
        ...
        // Aktuelle Artikel-Info erfassen
        ...
        // AJAX-Request mit Artikel-Info
        const formData = new FormData();
        formData.append('widget_id', 'article');
        formData.append('lazy_load', '1');
        
        // Artikel-Info hinzufügen, falls verfügbar
        if (articleInfo.articleId) {
            formData.append('info_center_article_id', articleInfo.articleId);
        }
        if (articleInfo.clangId) {
            formData.append('info_center_clang_id', articleInfo.clangId);
        }
        
        // AJAX-Request (vereinfacht - in echter Implementierung würde hier die korrekte API URL verwendet)
        ...
        .then(html => {
            // In einer echten Implementierung würde hier der HTML-Parser verwendet
            // Für jetzt: Nach kurzer Verzögerung wieder ursprünglichen Inhalt anzeigen
            setTimeout(() => {
                ...
                const event = new CustomEvent('infocenter:widget-refreshed', {
                    detail: { widget: widget, type: 'article', articleInfo: articleInfo }
                });
                document.dispatchEvent(event);
            }, 500);
        })
        .catch(error => {
            console.error('InfoCenter: Error refreshing article widget:', error);
            widgetContent.innerHTML = originalContent;
        });
    }
